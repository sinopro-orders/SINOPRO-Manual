...
|  번호 | 명칭            | 설명                                                                                               |
| :-: | ------------- | ------------------------------------------------------------------------------------------------ |
|  1  | 발생기 본체 전원 스위치 | 이 버튼을 통해 본체의 전원을 켜고 끌 수 있습니다.                                                                    |
|  2  | 영문 전환 버튼      | 이 버튼을 누르면 사용자 인터페이스를 영어 화면으로 전환할 수 있습니다.                                                         |
|  3  | 파라미터 설정 버튼    | 이 버튼을 누르면 비밀번호를 입력하여 파라미터 설정 화면으로 진입할 수 있습니다. 파라미터 설정 관련 내용은 그림 4를 참조해 주십시오. ※ 초기 비밀번호는 8888입니다. |
|  4  | 중문 전환 버튼      | 이 버튼을 누르면 사용자 인터페이스를 중국어 화면으로 전환할 수 있습니다.                                                        |
|  5  | USB 출력 포트     | USB 포트를 통해 각 용접 작업의 상세 데이터를 추출할 수 있습니다.                                                          |
...
|  번호 | 항목                      | 설명                                                                                                                                                                                         |
| :-: | ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
|  1  | 모드 (Mode)               | 장비의 현재 작동 상태를 표시합니다. (자동 모드, 금형 조정 모드 등 표시되며, 이 항목은 조정 불가입니다)                                                                                                                              |
|  2  | 전류 (Amplitude)          | 작업 시 발생하는 실제 전류 크기를 표시합니다. 기기의 출력 세기를 나타내며, 조정은 불가능합니다. ※ 무부하시 전류 수치를 통해 용접 헤드의 이상 유무도 판단할 수 있습니다.                                                                                         |
|  3  | 출력 (Power)              | 작업 중 장비의 실시간 출력값을 표시합니다. 매번 용접 시 출력 수치를 통해 용접 품질을 판단할 수 있습니다.                                                                                                                              |
|  4  | 에너지 (Energy)            | 각 용접 작업에서 소모된 에너지 크기를 표시합니다. 출력값과 함께 용접 품질 평가 기준으로 사용됩니다.                                                                                                                                  |
|  5  | 시간 (Time)               | 에너지 모드에서 각 용접 작업에 소요된 시간을 표시합니다. 작업 시간 기준으로 공정 일관성 확인이 가능합니다.                                                                                                                              |
|  6  | 주파수 (Frequency)         | 장비가 작동할 때마다 시스템이 자동으로 주파수를 추적하여 작동 안정성을 유지합니다. ※ 해당 항목은 사용자가 수동으로 조정할 수 없습니다.                                                                                                              |
|  7  | 생산량 (Production Number) | 장비가 한 번 용접을 완료할 때마다 1씩 증가하며, 총 작업 수량을 표시합니다.                                                                                                                                               |
|  8  | Freq Scanning           | 발생기(웰딩기)와 용접 헤드는 주파수를 맞춰야 합니다. 용접 헤드를 교체한 후 'Freq Scanning' 버튼을 누르면 장치가 자동으로 주파수를 보정하여 정상 작동을 보장합니다. 주파수로 재설정해야 합니다.                                                                       |
|  9  | 금형 조정 (Mould)           | 금형을 교체하거나 수평을 맞출 때 사용하는 모드로, 비작업 상태에서 오작동을 방지하기 위해 해당 버튼을 눌러 진입합니다.                                                                                                                        |
|  10 | 자동 (Automatic)          | 정상 용접 작업 시 사용하는 버튼으로, 이 버튼을 누르면 장비가 자동으로 용접을 시작합니다.                                                                                                                                        |
|  11 | 음파 테스트 (Sonic Test)     | 장비 설치 완료 후 또는 용접 헤드 교체 후 이 버튼을 눌러 현재 장치의 작동 주파수, 음파 발생 여부 및 전류 상태를 점검할 수 있습니다.                                                                                                             |
|  12 | 초기화 (Clear)             | 누적된 생산 수량 카운터를 초기화합니다. 버튼을 터치하면 생산량 수치가 0으로 변경.                                                                                                                                            |
|  13 | 리셋 (Reset)              | 장비가 과부하 상태일 때 이 버튼을 눌러 과부하 알람을 해제합니다. 버튼을 가볍게 터치하여 경고 상태를 초기화할 수 있습니다.                                                                                                                     |
|  14 | 진폭 (Amplitude)          | 장비의 진동 크기를 조절하는 항목입니다. 실제 작업 조건에 맞추어 설정할 수 있으며, 숫자 입력창을 터치해 값을 입력합니다. 작은 값에서 점차 큰 값으로 조정하는 것을 권장합니다.                                                                                       |
|  15 | 진동 해제 시간 (Shake Time)   | 발생기의 2차 발진 시간을 설정하여 용접 헤드가 공작물에 달라붙는 것을 방지합니다. 실제 작업 조건에 맞추어 설정하며, 숫자 입력창을 터치해 값을 입력합니다. 작은 값부터 순차적으로 늘려 설정하는 것을 권장합니다.                                                                    |
|  16 | 냉각 시간 (Cooling Time)    | 초음파 발생이 완료된 후 용접 헤드가 공작물 위에 머무르는 시간을 조정합니다. 숫자 입력창을 터치해 값을 입력하며, 작은 값에서 점진적으로 늘려 설정하는 것을 권장합니다.                                                                                            |
|  17 | 용접 에너지 (WELDING ENERGY) | 용접 과정에서 용접 헤드가 재료에 전달하는 총 에너지를 의미하며, 일반적으로 줄(Joule, J) 단위를 사용합니다.                                                                                                                          |
|  18 | 딜레이 (Delay)             | 용접 헤드가 하강하여 초음파를 발생하기 전까지의 시간. 이 시간 동안에는 초음파 발생기가 작동하지 않으며, 실제 용접 부품에 따라 적절히 설정 가능합니다. ※ 파라미터를 조정할 때는 숫자 프레임을 터치하여 입력하며, 작은 수치부터 점차 증가시키는 것을 권장합니다.                                        |
|  19 | 용접 모드 (Welding Mode)    | 에너지 모드와 시간 모드 중 선택할 수 있습니다. 에너지 모드는 '에너지 충족'에 중점을 두고, 시간 모드는 '시간 충족'에 중점을 둡니다. 실제 적용 시에는 재료 특성과 용접 요구사항에 따라 적절한 모드를 선택해야 합니다. ※ 파라미터를 조정할 때는 숫자 프레임을 터치하여 입력하며, 작은 수치부터 점차 증가시키는 것을 권장합니다. |
...
|  번호 | 명칭                   | 설명                                                                                             |
| :-: | -------------------- | ---------------------------------------------------------------------------------------------- |
|  1  | 실린더                  | 공기 공급 후 용접 헤드를 상하로 움직이는 부품으로, 하강 스트로크 조절 가능                                                    |
|  2  | 솔레노이드 밸브 (전자밸브)      | 메인 장비 전원 연결 시 실린더의 상하 운동을 제어하며, 공기 유입 및 배출량 조절 가능                                              |
|  3  | 압력 조절 밸브 (레귤레이터)     | 공기 주입구로, 공기 공급 후 압력 크기 조절 가능하며 동시에 수분 필터링 기능 포함                                                |
| 4·5 | 용접 헤드 (혼)            | 작업물에 직접 용접을 수행하는 부분이며, 사용 주파수에 따라 20K, 25K, 35K, 40K 등으로 구분. 용도에 따라 다양한 형상의 헤드 사용 가능 (도면 B 참조) |
|  6  | 하부 금형 패드             | 용접 몰드 하단에 위치하며, 형태는 용도에 따라 다양하게 설계 가능 (도면 A 참조)                                                |
|  7  | 하부 금형 지그             | 제품에 따라 다양한 형태의 금형 사용 가능 (도면 A 참조)                                                              |
|  8  | 리미트 스크류 (스트로크 제한 나사) | 용접 헤드의 상승 스트로크(상향 이동 거리)를 조절                                                                   |
|  9  | 고정 나사                | 진동자와 용접 헤드를 고정하는 나사로, 용접면 교체 시 사용                                                              |
|  10 | 진동자 연결 커넥터           | 발생기(메인 장치)와 연결되는 커넥터                                                                           |
|  11 | 솔레노이드 밸브 및 팬 연결 커넥터  | 팬 및 발생기를 통해 전자밸브를 구동하는 전원 연결용                                                                  |
...
|  번호 | 명칭                         | 설명                                                               |
| :-: | -------------------------- | ---------------------------------------------------------------- |
|  1  | 풋 스위치 포트                   | 풋 스위치를 연결하는 단자입니다. 이 단자는 자동 공급 장치에서 메인 장치로 트리거 신호를 보내는 역할도 겸합니다. |
|  2  | 전원 케이블 포트                  | 전원 케이블을 연결하는 포트입니다. 입력 전원은 기본적으로 AC 220V를 사용합니다.                 |
|  3  | 퓨즈 (15A)                   | 기기를 보호하는 역할을 하며, 과전류 또는 과부하 시 회로 차단 기능을 합니다.                     |
|  4  | 트랜스듀서 포트 (진동자 연결 포트)       | 초음파 트랜스듀서(진동자)를 연결하는 9핀 커넥터 포트입니다.                               |
|  5  | 냉각 팬                       | 장비 내부의 발열을 줄이기 위한 방열 팬입니다.                                       |
|  6  | 솔레노이드 밸브 팬 포트 (전자밸브 연결 포트) | 솔레노이드 밸브를 연결하는 10핀 커넥터 포트입니다.                                    |
...
{% hint style="info" %}
원본 문서에 별도의 사양표(TDS)가 없습니다. 본문에서 확인된 참고 수치는 다음과 같으며, 정식 사양은 추후 보완이 필요합니다.
{% endhint %}

| 항목     | 본문 참고 수치                                |
| ------ | --------------------------------------- |
| 입력 전원  | AC 220V / 50Hz                          |
| 작동 주파수 | 약 40kHz (혼 종류: 20K / 25K / 35K / 40K 등) |
| 무부하 전류 | 약 0.3A (정상 시)                           |
| 권장 공압  | 2\~3kg/cm²                              |
| 에어 호스  | M8 규격 권장                                |
| 퓨즈     | 15A                                     |
